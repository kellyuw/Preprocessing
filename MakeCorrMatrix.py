...
subject_dir = project_dir + '/' + str(subject) + '/session' + str(session)
warped_roi_ts = glob.glob(subject_dir + '/xfm_dir/rest/yeo_labels/*.txt')
...
def get_roi_df(x):
    if x not in custom_roi_ts:
        if len(os.path.basename(x).split('_')) == 5:
            roi_num, roi_extra, roi_hemi, roi_network, roi_raw_region = os.path.basename(x).split('_')
            name = '_'.join([roi_hemi, roi_network, roi_raw_region.replace('.txt','')])
        else:
            roi_num, roi_extra, roi_hemi, roi_network = os.path.basename(x).split('_')
