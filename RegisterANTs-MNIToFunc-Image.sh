...
if [ $# -lt 2 ]; then
	echo
	echo   "bash RegisterANTs-Image.sh <image type - label, image> <full path to MNI image> <full path to func image>"
	echo
	exit 1
fi
...
if [[ ${IMAGE_TYPE} == *label* ]]; then
	INTERP="-n MultiLabel"
fi

if [[ ${PROJECT} == *PING* ]] || [[ ${PROJECT} == *HOME* ]]; then
	echo "Warping ${MNI_IMAGE} to ${FUNC_IMAGE}"
	${ANTSpath}/antsApplyTransforms -i ${MNI_IMAGE} -r ${FUNC_BRAIN} -t  [${MNIREGPREFIX}_0GenericAffine.mat,1] ${MNIREGPREFIX}_1InverseWarp.nii.gz [${CUSTOMREGPREFIX}_0GenericAffine.mat,1] ${CUSTOMREGPREFIX}_1InverseWarp.nii.gz ${FUNCREGPREFIX}_1Warp.nii.gz ${FUNCREGPREFIX}_0GenericAffine.mat ${INTERP} -o ${OUTPUT}

elif [[ ${PROJECT} == *fear_pipeline* ]]; then
	echo "Warping ${MNI_IMAGE} to ${FUNC_IMAGE}"
	${ANTSpath}/antsApplyTransforms -i ${MNI_IMAGE} -r ${FUNC_BRAIN} -t [${MNIREGPREFIX}_0GenericAffine.mat,1] ${MNIREGPREFIX}_1InverseWarp.nii.gz [${CUSTOMREGPREFIX}_0GenericAffine.mat,1] ${CUSTOMREGPREFIX}_1InverseWarp.nii.gz [${FUNCREGPREFIX}_0GenericAffine.mat,1] ${FUNCREGPREFIX}_1InverseWarp.nii.gz ${INTERP} -o ${OUTPUT}
fi
